...
| **Global system prompt** | Settings → Plugins → Web Enhanced → **Global prompt**: one switch and one text block saved into the `dsh-web-enhanced-global-prompt` settings namespace. The host registers the text as a global `systemPrompt` section (order 50 — after the selected agent preset's persona, before tool guidance), so it is appended to **every mode, session, and subagent**. The section text is read per assembly: a save reaches the next model request immediately, with no `dsh web` restart; switching it off or leaving it empty injects nothing. (The Minimal preset declares its persona `complete` — it replaces the whole system prompt — so the global prompt is not appended there.) |
| **Workspace view** | A **Workspace** tab in the conversation's view ring, beside Chat and Trajectory, with four panes (**Explorer** / Changes / **Task Board** / **Git Graph**). The Explorer is VSCode-style: a collapsible file-tree sidebar on the left (collapse state persists) and the open file's preview on the right; the tree expands, searches by name, and opens files into that preview side; preview supports markdown (GFM tables, HTML tables, and inline HTML) / HTML (sandboxed iframe) / code / **diff** (line-highlighted unified diff) / CSV / images / PDF / text / **Office docx & xlsx** (host-side structural conversion) with source / **split** (editor + preview side by side) / view modes and save. The Changes pane is backed by real `git status` with stage / unstage / discard and per-file diffs. The active pane and the open directories persist per workspace. A **terminal drawer** (below) sits along the bottom of the view, under all four panes.
| **Integrated terminal** | A **drag-resized drawer along the bottom of the Workspace view**, present across all four tabs — type a command while still looking at the file tree, changes, board, or graph, instead of switching to an editor. A tab strip in the drawer opens, switches, and closes terminals, so a dev server can stay up while you run something else. The PTY comes from the host's `subprocess.spawnTerminal` (node-pty), starts in the project root, and runs **your own shell** (rc and profile are not disabled, so aliases and prompt are intact) under `TERM=xterm-256color`, so color works and full-screen programs like vim and htop are usable. **Sessions live in the dsh process, not in the page**: a reload, a tab switch, or a dropped connection does not kill them — reconnecting re-attaches by id and replays the last 256 KB of output, with `cd` and running processes still there. The drawer starts collapsed; its height, collapse state, and focused terminal persist. |
...
Plugin-row `config` fields (all have defaults; the `vision*` ones can also be edited live in Settings → Plugins → Web Enhanced → Vision, whose saves override these base values):
...
| `terminalShell` | empty | Shell the integrated terminal starts; empty uses `$SHELL` (`%COMSPEC%` on Windows), falling back to `/bin/bash` |
| `terminalScrollbackBytes` | 256 KiB | Output retained per terminal and replayed when a browser re-attaches |
| `terminalGraceMs` | 5000 | TERM-to-KILL grace for a terminal's whole session process tree on close |
| `terminalTrustedHosts` | `[]` | **Non-loopback** authorities (`host:port`) allowed to open a terminal. Empty means this machine only; **adding one hands a shell to anything that can reach this deployment under that name** |
...
- **Plugin management modifies no host file**: the settings page registers into the existing `settings.plugins.tab` slot, and the inventory rides this plugin's own Typert gateway — so unlike DSH-vision it needs no edit to the api-proxy's settings allowlist (that patches the host's published output inside `node_modules`, which every upgrade overwrites). Remove and update only run pnpm in the profile directory and rewrite that profile's `dsh.profile.bundles`, exactly the path `dsh plugin` takes. `@deepseek-ai/dsh-app-boot`, which owns those routines for the CLI, is deliberately NOT a peer dependency: it belongs to the dsh installation rather than the profile, so peer resolution would fail in precisely the deployment this code runs in.
- **The terminal sits on `subprocess.spawnTerminal`, not the host's `ctx.terminals`**: the latter looks like the better fit (it *is* a persistent PTY registry), but its output goes through `TerminalSanitizer`, which strips every CSI/OSC escape and leaves line text — no color, no cursor control, and no full-screen program like vim. It also scopes each session to one live `Agent` (these terminals belong to the person at the browser, not to a model turn), permits one `startSend` at a time, and its packages (`dsh-terminal`, `dsh-terminal-bash`) are not in the web profile's default bundle. The lower subprocess seam delivers raw terminal bytes, leaves session lifetime to its caller, and `ctx.subprocess` is already a dependency of this plugin.
- **The terminal brings its own bidirectional WebSocket**: Typert has only unary invocations (no streaming invocation kind), the host's `/api/events.mux` and `/api/events.host` are downlink-only (any client frame is met with `close(1008)`) and closed to new frame types, and the board's 2-second polling cannot make a terminal you can type into. So the drawer claims `/plugins/dsh-web-enhanced/terminal` through the public `webServer.registerUpgrade` seam and completes the handshake itself with `ws`. Frames deliberately carry no envelope: every client frame is keystrokes, every server frame is terminal output, and the control plane (list, open, close, signal) stays on unary Typert methods. The route mounts via `ctx.inject(['webServer'])` rather than the plugin's `inject`, so a headless deployment still loads this plugin.
- **Both terminal fences are at the handshake**: the `Host` header defeats DNS rebinding, and the `Origin` header defeats cross-site WebSocket hijacking — **WebSocket is exempt from CORS entirely**, so without the Origin check any page on the internet could `new WebSocket('ws://localhost:3190/...')` and own the user's shell. Browsers always send Origin for WebSocket, so a missing one is refused too.
...
```sh
pnpm install
pnpm run check   # typecheck + full tests + build (544 tests, 2 skipped)
```
...
- **A terminal's size is fixed when it is created and cannot change afterwards**: the host's `SubprocessTerminalHandle` has no resize method (only `write` / `signalForeground` / `terminate`), so a PTY's rows and columns can only be set at spawn. Dragging the drawer's height only changes how many rows you can see, which ordinary command lines do not care about; but **after the browser window's width changes**, the column count no longer matches the PTY and full-screen programs like vim or htop will render wrong — open a new terminal. A real fix means adding `resize()` to the host's subprocess seam (node-pty supports it).
- **The terminal has no approval gate**: commands execute directly, consistent with this plugin's git and pnpm operations, which already run subprocesses outside `ctx.approval`. The only defense is the Host + Origin check at the WebSocket handshake: by default only same-origin loopback pages are accepted, and a non-loopback authority needs an explicit `terminalTrustedHosts` entry. **Exposing the dsh web port to a LAN or the internet and adding a trusted host exposes the shell with it.**
- **Terminals do not survive the process**: PTYs live in the dsh process with no disk persistence, so a restart loses them all. The replay buffer defaults to 256 KB (`terminalScrollbackBytes`); older output is dropped, and a replay can begin mid-escape-sequence (terminals discard an incomplete sequence, exactly as a real scrollback does).
- **The terminal does not exist in a deployment without a web server**: its WebSocket route mounts through `ctx.inject(['webServer'])`, so a headless deployment loads this plugin normally and simply has no terminal.
