...
- **Zero harness changes**: the client UI only registers into existing slots.
  - `conversation.view` — the Workspace tab, one entry in the view ring beside Chat and Trajectory. The ring renders one view at a time at full column width, so this surface owns no geometry: no docking, no drag-to-resize, no collapse. Those belong to the frame. Its internal tablist carries Files / Preview / Changes / **Task Board** / **Git Graph**.
