...
| **Balance line** | The DeepSeek-aware info row below the composer, auto-refreshed every minute. It shows the provider/model display names read from the same LLM directory as the model picker, the CNY balance from `GET /user/balance` (hover: granted / topped-up amounts), a ⚠ warning below ¥20, the current Beijing **peak / off-peak price period** with a countdown to the next switch (V4 Flash/Pro peak-valley; legacy chat flat), and **this conversation's estimated cost** (DeepSeek CNY rates for V4 models, models.dev USD elsewhere, `¥0.000` while the session is fresh). Failed refreshes keep the last good snapshot and mark it stale instead of blinking away. **Only while the session's model route actually bills that account** — switching to another channel (or repointing `deepseek-official` at a private gateway) hides the row, because the number would then be about somebody else's account. |
