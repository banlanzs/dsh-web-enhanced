 ...
 */
function isProfileManifest(manifest: Record<string, unknown>): boolean {
 ...
 */
export async function findProfileDir(from?: string): Promise<string | undefined> {
  ...
  for (let depth = 0; depth < 40; depth += 1) {
    ...
    if (manifest !== undefined && isProfileManifest(manifest)) return dir
...
export async function readProfileManifest(profileDir: string): Promise<ProfileManifest> {
  ...
  if (manifest === undefined) throw new Error(`profile manifest unreadable at ${profileDir}`)
 ...
 */
async function readInstalled(profileDir: string, packageName: string): Promise<Record<string, unknown> | undefined> {
 ...
 */
function declaresBundle(manifest: Record<string, unknown> | undefined): boolean {
  ...
  return (bundle as Record<string, unknown>)['patch'] !== undefined
}
 ...
 */
async function selfPackageDir(): Promise<string | undefined> {
 ...
 */
export async function readInventory(profileDir: string): Promise<ProfileInventory> {
  const manifest = await readProfileManifest(profileDir)
  const dependencies = manifest.dependencies ?? {}
  const bundles = manifest.dsh?.profile?.bundles ?? []
  const selfDir = await selfPackageDir()
  const names = Object.keys(dependencies)
  const plugins = await Promise.all(names.map(async (name): Promise<PluginRow> => {
...
export async function reconcileBundles(
  profileDir: string,
  beforeDependencies: readonly string[],
): Promise<{ readonly added: readonly string[]; readonly removed: readonly string[] }> {
  const manifest = await readProfileManifest(profileDir)
  ...
  const isBundle = new Map<string, boolean>()
 ...
 */
export function profilePath(profileDir: string, ...segments: string[]): string {
