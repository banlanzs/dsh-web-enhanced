...
import { GLOBAL_PROMPT_SETTINGS_NS } from './types.ts'
...
export const GlobalPromptSettingsSchema: z<GlobalPromptSettingsValue> = z.object({
 ...
 */
export function globalPromptTextOf(value: unknown): string {
  if (typeof value !== 'object' || value === null) return ''
  const settings = value as { readonly enabled?: unknown; readonly text?: unknown }
 ...
 */
export function applyGlobalPrompt(ctx: Context): void {
  const settings = ctx.get('settings' as never, false) as unknown as GlobalPromptSettingsServiceFace | undefined
  if (settings === undefined || typeof settings.register !== 'function') return
  let scope: GlobalPromptSettingsScopeFace
  try {
