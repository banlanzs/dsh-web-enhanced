...
export const DEEPSEEK_RATES: Readonly<Record<string, DeepSeekRateEntry>> = {
  'deepseek-v4-flash': {
    mode: 'peak-valley',
...
export function beijingMinutes(nowMs: number): number {
...
export function currentDeepSeekPeriod(nowMs: number): 'peak' | 'offpeak' {
 ...
 * tomorrow 09:00.
 */
export function nextDeepSeekSwitchAt(nowMs: number): number {
...
export function nextDeepSeekSwitchLabel(nowMs: number): string {
 ...
 */
export function deepseekRateFor(model: string, nowMs: number = Date.now()): DeepSeekRateView {
