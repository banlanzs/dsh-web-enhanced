...
import type { TerminalRegistry, TerminalSink } from './terminal.ts'
import { TERMINAL_EXIT_CLOSE_CODE, TERMINAL_GONE_CLOSE_CODE, TERMINAL_SOCKET_PATH } from './types.ts'

/** The narrow face of the host web server this module uses. */
 ...
 */
export function isTrustedTerminalUpgrade(
  req: IncomingMessage,
 ...
 */
export function applyTerminalSocket(
  ctx: Context,
  registry: TerminalRegistry,
  options: TerminalSocketOptions,
): void {
  ctx.inject(['webServer'], (webCtx: Context) => {
    const server = webCtx.get('webServer' as never) as unknown as WebServerFace | undefined
...
function attach(connection: WebSocket, registry: TerminalRegistry, terminalId: string): void {
