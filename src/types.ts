...
export const GLOBAL_PROMPT_SETTINGS_NS = 'dsh-web-enhanced-global-prompt' as const
 ...
 */
export const TERMINAL_SOCKET_PATH = '/plugins/dsh-web-enhanced/terminal' as const

/** Close code telling the browser the shell exited, so it must not reconnect. */
export const TERMINAL_EXIT_CLOSE_CODE = 4000 as const

/** Close code for a refused attach (unknown session), so it must not reconnect. */
export const TERMINAL_GONE_CLOSE_CODE = 4001 as const
...
export interface TerminalListRequest { readonly workspaceId: string }
...
export interface TerminalCloseRequest { readonly terminalId: string }
