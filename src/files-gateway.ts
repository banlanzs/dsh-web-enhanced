...
import { countTextLines, deleteFileView, listDirectory, readFileView, searchFiles, writeFileView } from './files.ts'
...
import type {
  ...
  FsOfficePreviewRequest, FsOfficePreviewResult, FsReadRequest, FsReadResult,
  FsSearchRequest, FsSearchResult, FsWriteRequest, FsWriteResult,
} from './types.ts'
...
export const filesConfigFragment: z<Required<FilesConfigInput>> = z.object({
...
export function resolveFilesConfig(config: Partial<FilesConfigInput>): Required<FilesConfigInput> {
  return {
 ...
 */
export function createFilesDomain(deps: FilesDomainDeps): FilesDomainFace {
  /**
   ...
   */
  const emptySearchCache = new Map<string, { at: number; result: FsSearchResult }>()

  const fsLimits = (): FsLimits => ({
  ...
  const officeLimits = (): OfficeLimits => ({ maxBytes: deps.config.officeMaxBytes })
   ...
   */
  const storeEmptySearch = (key: string, result: FsSearchResult): void => {
   ...
   */
  const invalidateSearchCache = (workspaceId: string): void => {
    const prefix = `${workspaceId}:`
    for (const key of [...emptySearchCache.keys()]) {
      if (key.startsWith(prefix)) emptySearchCache.delete(key)
    }
  }
  ...
  return {
    ...
    async search(request) {
      ...
      try {
        const result: FsSearchResult = { entries: await searchFiles(root, rel, request.query ?? '', fsLimits()) }
