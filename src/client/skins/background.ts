...
const STEPS: readonly EncodeStep[] = [
 ...
 */
export function compressionPlan(): readonly EncodeStep[] {
 ...
 */
export function approxBytesOf(dataUrl: string): number {
...
/** Decoded source image ready to draw. */
 ...
 */
function encode(image: DecodedImage, step: EncodeStep): string | undefined {
 ...
 */
export async function encodeBackground(file: Blob): Promise<string> {
  const decoded = await decode(file)
  try {
    for (const step of compressionPlan()) {
      const encoded = encode(decoded, step)
      if (encoded !== undefined && fitsBudget(encoded)) return encoded
    }
  } finally {
    decoded.release()
  }
