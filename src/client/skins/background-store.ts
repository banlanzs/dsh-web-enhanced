 ...
 */
async function transact<T>(mode: IDBTransactionMode, run: (blobs: IDBObjectStore) => IDBRequest<T>): Promise<T> {
...
export function blobToDataUrl(blob: Blob): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      else reject(new Error('data URL read produced a non-string result'))
    }
