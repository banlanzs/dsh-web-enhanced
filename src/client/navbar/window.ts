 ...
 */
export function olderNodeCount(
 ...
 */
export function olderWindow(count: number, max: number = MAX_OLDER_DOTS): { readonly hidden: number; readonly visible: number } {
...
export function navWindow(
  ...
  windowSize: number = NAV_WINDOW,
  halfWindow: number = NAV_HALF_WINDOW,
): NavWindow {
  if (count <= 0) return { lo: 0, hi: -1 }
  if (count <= windowSize) return { lo: 0, hi: count - 1 }
