 ...
 */
export function visibleCapabilityProvider(
  entry: ConfigurableProviderView,
  namespace: SettingsNamespaceView | undefined,
): boolean {
  if (entry.settingsNs === DEEPSEEK_NS) return true
  if (entry.settingsNs !== PI_AI_NS) return false
...
export function modelOptionsOf(
  ...
  for (const layer of [namespace.value, namespace.base]) {
    ...
    for (const entry of models) {
      ...
      push((entry as Record<string, unknown>)['id'], (entry as Record<string, unknown>)['name'])
    }
  }
  for (const layer of [namespace.value, namespace.base]) {
    ...
    for (const id of Object.keys(overrides)) push(id, undefined)
  }
  return options
}
