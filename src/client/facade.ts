...
export function createRemoteFacade(
  ...
  return {
    ...
    // transport failure becomes an unavailable view rather than a foreign shape.
    balanceGet: async (): Promise<BalanceView> => {
      const settled = await raw.balanceGet()
      return settled.ok
        ? settled.value
        : { isAvailable: false, infos: [], cachedAt: now(), error: apiErrorOf(settled.error) }
    },
