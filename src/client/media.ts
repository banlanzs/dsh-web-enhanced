...
function bytesOfBase64(base64: string): Uint8Array<ArrayBuffer> {
 ...
 */
export function binaryObjectUrl(key: string, base64: string, mime: string): string {
  ...
  while (urls.size > URL_CAPACITY) {
    ...
    if (url2 !== undefined) URL.revokeObjectURL(url2)
  }
  return url
}
 ...
 */
export function contentKey(prefix: string, binary: string): string {
...
export function releaseObjectUrl(key: string): void {
  ...
  urls.delete(key)
  if (objectUrlsAvailable()) URL.revokeObjectURL(url)
}
...
export interface ImageReadFace {
  ...
  fsRead(request: { readonly workspaceId: string; readonly path: string }): Promise<
    | { readonly kind: 'binary'; readonly content: string }
...
export function workspaceImageUrl(
  remote: ImageReadFace,
  workspaceId: string,
  path: string,
  mimeOfPath: (path: string) => string,
): Promise<string> {
