 ...
 */
export function browserImageHref(href: string): string | undefined {
  const trimmed = href.trim()
 ...
 */
export function workspaceImagePathOf(markdownPath: string, href: string): string | undefined {
  const trimmed = href.trim()
  if (trimmed === '' || browserImageHref(trimmed) !== undefined) return undefined
  if (/^[\\/]/u.test(trimmed) || /^[a-zA-Z]:/u.test(trimmed)) return undefined
