 ...
 */
export function pastedTextClipboard(ref: string): string {
  return `[已粘贴文本:${ref.slice(0, 8)}]`
}

/** Parse persisted entries, dropping every row that is not the stored shape. */
export function revivePastedText(raw: unknown): PastedTextEntry[] {
  if (!Array.isArray(raw)) return []
  const seen = new Set<string>()
  const entries: PastedTextEntry[] = []
...
export class PastedTextStore {
  private entries = new Map<string, PastedTextEntry>()
