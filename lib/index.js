...
const WEB_ENHANCED_DESCRIPTORS = [
	...
	nullary("visionStatus", "VisionStatusResult", okOrError(z.object({
		mounted: z.boolean(),
		enabled: z.boolean(),
...
var VisionTranscriber = class {
	...
	constructor(settings, deps) {
		...
		this.deps = deps;
		this.ollamaProbe = settings.autoLocalOllama ? detectLocalOllama(this.fetchImpl, settings.localOllamaUrl, OLLAMA_PROBE_TIMEOUT_MS, settings.localOllamaModel).then((local) => local === null ? null : {
			model: local.model,
...
//#endregion
export { Config, VisionInterceptor, VisionTranscriber, WebEnhancedGateway, apply, inject, name };
