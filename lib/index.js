...
const deepseekRateWindowSchema = z.object({
	inputCacheHit: z.number(),
	inputCacheMiss: z.number(),
	output: z.number()
});
const deepseekRateViewSchema = z.object({
	model: z.string(),
	mode: z.enum([
		"peak-valley",
		"flat",
	...
	nextSwitchAt: z.number().nullable(),
	nextSwitchLabel: z.string().nullable(),
	nextIsPeak: z.boolean(),
	now: z.number()
});
...
const opencodeGoUsageViewSchema = z.object({
	provider: z.literal("opencode-go"),
	plan: z.literal("OpenCode Go"),
	windows: z.array(opencodeGoWindowSchema),
...
const WEB_ENHANCED_DESCRIPTORS = [
	...
	unary("modelRouteDescribe", "ModelRouteDescribeRequest", "ModelRouteDescribeResult", okOrError(z.object({
		provider: z.string(),
		model: z.string(),
	...
	nullary("modelRetryGet", "ModelRetryGetResult", okOrError(z.object({ configs: z.array(z.object({
		provider: z.string(),
	...
	unary("memoryList", "MemoryListRequest", "MemoryListResult", okOrError(z.object({ memories: z.array(memoryRecordSchema) }))),
	unary("memoryDelete", "MemoryDeleteRequest", "MemoryDeleteResult", okOrError(z.object({ removed: z.boolean() }))),
	nullary("memoryConfigGet", "MemoryConfigGetResult", okOrError(z.object({
		enabled: z.boolean(),
		revision: z.number().nullable(),
		writable: z.boolean()
	}))),
	unary("memoryConfigSet", "MemoryConfigSaveRequest", "MemoryConfigSetResult", okOrError(z.object({
		ok: z.literal(true),
		revision: z.number()
	}))),
...
*/
async function migrateJsonDomainV1ToV2(ctx) {
	const facility = ctx.get("storageDomain", false);
	const hub = ctx.get("storage", false);
	if (facility === void 0 || hub === void 0) return;
	...
	header["version"] = 2;
	const tableRecord = tables;
	if (tableRecord["memories"] === void 0) tableRecord["memories"] = {};
	try {
...
const sharedDomains = /* @__PURE__ */ new WeakMap();
/**
...
*/
async function openSharedDomain(ctx) {
...
const boardConfigFragment = z$1.object({ cronIntervalMs: z$1.number().default(3e4) });
/** Field defaults applied when the board is assembled directly. */
function resolveBoardConfig(config) {
	return { cronIntervalMs: config.cronIntervalMs ?? 3e4 };
}
/**
	...
	*/
	armScheduler(domain) {
		...
		for (const [, task] of domain.table("tasks").entries()) {
			...
			if (earliest === void 0 || task.nextRunAt < earliest) earliest = task.nextRunAt;
		}
	...
	disarmScheduler() {
		if (this.schedulerTimer === void 0) return;
...
//#endregion
//#region lib/types/workspace-service.js
/**
...
*/
function createWorkspaceFace(ctx) {
...
//#endregion
//#region lib/types/files-gateway.js
/**
...
const FS_SEARCH_CACHE_TTL_MS = 3e4;
...
const filesConfigFragment = z$1.object({
...
function resolveFilesConfig(config) {
	return {
...
*/
function createFilesDomain(deps) {
	/**
	...
	*/
	const emptySearchCache = /* @__PURE__ */ new Map();
	const fsLimits = () => ({
	...
	*/
	const storeEmptySearch = (key, result) => {
		...
		emptySearchCache.set(key, {
			at: Date.now(),
			result
		});
		while (emptySearchCache.size > FS_SEARCH_CACHE_LIMIT) emptySearchCache.delete(emptySearchCache.keys().next().value);
	};
	/**
	...
	*/
	const invalidateSearchCache = (workspaceId) => {
		...
		for (const key of [...emptySearchCache.keys()]) if (key.startsWith(prefix)) emptySearchCache.delete(key);
	};
	return {
		countLines: (root, path) => countTextLines(root, path, fsLimits()),
		async list(request) {
			const root = deps.workspace.rootFor(request.workspaceId);
			if (root === null) return { error: workspaceNotFound(request.workspaceId) };
			try {
		...
		async search(request) {
			...
			if (cacheable) {
				...
				if (hit !== void 0 && Date.now() - hit.at < FS_SEARCH_CACHE_TTL_MS) return hit.result;
			}
			try {
				const result = { entries: await searchFiles(root, rel, request.query ?? "", fsLimits()) };
				if (cacheable) storeEmptySearch(key, result);
				return result;
			} catch (error) {
...
//#endregion
//#region lib/types/git-gateway.js
/**
...
const gitConfigFragment = z$1.object({
...
function createGitDomain(deps) {
	...
	return {
		...
		log: (request) => withGit(request.workspaceId, async (client) => {
			const maxCount = request.maxCount === void 0 ? deps.config.gitMaxCount : request.maxCount;
		...
		commitDiff: (request) => withGit(request.workspaceId, async (client) => ({ text: await client.commitDiff(request.hash, request.path) })),
		working: (request) => withGit(request.workspaceId, async (client, root) => ({ working: await client.working(deps.config.gitWorkingMaxFiles, (path) => deps.countLines(root, path)) })),
		...
		diff: (request) => withGit(request.workspaceId, async (client) => ({ text: await client.diff(request.path, request.staged === true) })),
...
var BalanceClient = class {
	...
	*/
	...
	async get() {
		...
		if (view.error === void 0) this.lastGood = view;
		return view;
	}
	...
	*/
	...
	async fetchBalance(now) {
		...
		if (key === void 0) return this.failure(now, "no-api-key", `credential ${this.config.apiKeyEnv} is not configured (checked the credential store and the environment)`);
		let response;
		try {
		...
		if (!response.ok) return this.failure(now, "balance-http", `balance endpoint answered ${response.status}`);
		let body;
		try {
...
//#endregion
//#region lib/types/deepseek-rate.js
/**
...
function beijingMinutes(nowMs) {
	return new Date(nowMs + 288e5).getUTCHours() * 60 + new Date(nowMs + 288e5).getUTCMinutes();
}
/** Whether `nowMs` falls in a Beijing peak window. */
function currentDeepSeekPeriod(nowMs) {
...
* tomorrow 09:00.
*/
function nextDeepSeekSwitchAt(nowMs) {
	const beijing = new Date(nowMs + 288e5);
	...
	for (const bound of [
		...
		840,
		1080
	]) if (bound > minutes) return Date.UTC(beijing.getUTCFullYear(), beijing.getUTCMonth(), beijing.getUTCDate(), Math.floor(bound / 60), 0, 0) - 288e5;
	return Date.UTC(beijing.getUTCFullYear(), beijing.getUTCMonth(), beijing.getUTCDate() + 1, 9, 0, 0) - 288e5;
}
/** `HH:MM` Beijing label of the next switch. */
function nextDeepSeekSwitchLabel(nowMs) {
	const at = nextDeepSeekSwitchAt(nowMs);
	const beijing = new Date(at + 288e5);
...
*/
function deepseekRateFor(model, nowMs = Date.now()) {
...
*/
function settingsFace(ctx) {
	return ctx.get("settings", false);
}
//#endregion
//#region lib/types/model-names.js
/**
...
var ModelRouteNames = class {
	llm;
	providerNames = /* @__PURE__ */ new Map();
	modelNames = /* @__PURE__ */ new Map();
	pendingModels = /* @__PURE__ */ new Map();
	/**
	...
	*/
	...
	async providerName(provider) {
		...
		return this.providerNames.get(provider) ?? PROVIDER_DISPLAY[provider] ?? (provider.length > 0 ? provider.charAt(0).toUpperCase() + provider.slice(1) : provider);
	}
	...
	async loadProviders() {
		if (this.llm === void 0 || typeof this.llm.listProviders !== "function") return;
		try {
			for (const provider of this.llm.listProviders()) if (typeof provider.name === "string" && provider.name.length > 0) this.providerNames.set(provider.id, provider.name);
		} catch {}
	}
	loadModels(provider) {
		...
		const loading = (async () => {
			if (this.llm === void 0 || typeof this.llm.listModels !== "function") return;
			try {
				const models = await this.llm.listModels(provider);
				const names = /* @__PURE__ */ new Map();
				for (const model of models) if (typeof model.name === "string" && model.name.length > 0) names.set(model.id, model.name);
				this.modelNames.set(provider, names);
			} catch {}
		})();
		this.pendingModels.set(provider, loading);
		loading.finally(() => {
			this.pendingModels.delete(provider);
		});
		return loading;
	}
};
//#endregion
//#region lib/types/opencode-go.js
/**
...
*/
function normalizeOpencodeGoResetAt(value) {
...
*/
function opencodeGoKeyFromAuth(raw) {
...
*/
var OpencodeGoUsageClient = class {
	config;
	resolveCredential;
	cached = null;
	cachedAt = 0;
	...
	*/
	...
	async apiKey() {
		...
		const ambient = process.env[this.config.apiKeyEnv];
		if (ambient !== void 0 && ambient.trim() !== "") return ambient;
	...
	async fetch() {
		...
		const key = await this.apiKey();
		if (key === void 0) return this.failure("opencode-go-no-key", "OPENCODE_GO_API_KEY is not configured and the opencode CLI has no opencode-go login", previous);
		let response;
		try {
		...
		if (!response.ok) return this.failure("opencode-go-http", `the OpenCode Go usage endpoint answered ${response.status}`, previous);
		let body;
		try {
			body = await response.json();
		} catch {
		...
		if (windows === null) return this.failure("opencode-go-invalid", "unexpected OpenCode Go usage payload", previous);
		return {
			provider: "opencode-go",
			plan: "OpenCode Go",
			windows,
...
//#endregion
//#region lib/types/model-gateway.js
/**
...
const MODEL_RETRY_NAMESPACES = /* @__PURE__ */ new Set(["llm-deepseek", "llm-pi-ai"]);
/**
...
*/
function retryConfigurableEntries(llm) {
	const active = new Set(llm.listProviders().map((entry) => entry.id));
	return llm.listConfigurableProviders().filter((entry) => MODEL_RETRY_NAMESPACES.has(entry.settingsNs) && (entry.declared === true || active.has(entry.provider)));
}
...
const modelConfigFragment = z$1.object({
	...
	opencodeGoUsageUrl: z$1.string().default("https://opencode.ai/zen/go/v1/usage"),
	opencodeGoCacheTtlMs: z$1.number().default(6e4),
	opencodeGoAuthFile: z$1.string().default("")
});
...
function resolveModelConfig(config) {
	return {
		...
		opencodeGoCacheTtlMs: config.opencodeGoCacheTtlMs ?? 6e4,
...
*/
function createModelDomain(deps) {
	const { ctx } = deps;
	...
	const routeNames = new ModelRouteNames(ctx.get("llm", false));
	...
	*/
	const applies = (provider) => balanceApplies({
		provider,
		allowed: deps.config.balanceProviders,
		balanceBaseUrl: deps.config.balanceBaseUrl,
		providerBaseUrl: provider === void 0 ? void 0 : providerBaseUrl(provider)
	});
	return {
		clearRouteNames: () => {
			routeNames.clear();
		...
		async balance(request) {
			if (!applies(request.provider)) return {
				applicable: false,
				isAvailable: false,
				infos: [],
				cachedAt: Date.now()
		...
		async retryGet() {
			try {
				...
				if (settings === void 0) return { error: {
					code: "model-retry-settings-unavailable",
					message: "the settings service is not mounted in this deployment"
				} };
				const llm = ctx.get("llm");
				if (llm === void 0) return { configs: [] };
				...
				for (const entry of retryConfigurableEntries(llm)) {
					let section;
					try {
						section = readPath(settings.get(entry.settingsNs), entry.settingsPath);
					} catch {
						section = void 0;
					}
					...
					const displayName = typeof route["displayName"] === "string" && route["displayName"].length > 0 ? route["displayName"] : null;
					configs.push({
						...
						maxDelayMs: typeof backoff["maxDelayMs"] === "number" ? backoff["maxDelayMs"] : 1e4,
						jitterRatio: typeof backoff["jitterRatio"] === "number" ? backoff["jitterRatio"] : .1
					});
				}
				return { configs };
			} catch (error) {
		...
		async retrySet(request) {
			try {
				if (!Number.isSafeInteger(request.maxRetries) || request.maxRetries < 0) return { error: {
					code: "model-retry-invalid",
					message: "maxRetries must be a non-negative safe integer"
				} };
				const settings = settingsFace(ctx);
				if (settings === void 0) return { error: {
					code: "model-retry-settings-unavailable",
				...
				if (!settings.writable) return { error: {
					code: "model-retry-settings-readonly",
					message: "the settings provider is read-only"
				} };
				const llm = ctx.get("llm");
				const entry = llm === void 0 ? void 0 : retryConfigurableEntries(llm).find((candidate) => candidate.provider === request.provider);
				if (entry === void 0) return { error: {
					code: "model-retry-unmanaged",
					message: `provider "${request.provider}" has no configurable retry settings`
				} };
				await settings.update(entry.settingsNs, nestAt(entry.settingsPath, { retryPolicy: {
					mode: "normal",
					maxRetries: request.maxRetries
				} }), request.expectedRevision);
				return {
					ok: true,
					revision: settings.describe().find((desc) => desc.ns === entry.settingsNs)?.revision ?? 0
				};
			} catch (error) {
				return { error: errorOf(error, error.code === "SETTINGS_CONFLICT" ? "model-retry-conflict" : "model-retry-save") };
			}
		}
	};
}
...
//#endregion
//#region lib/types/plugins-gateway.js
/**
...
const pluginsConfigFragment = z$1.object({
...
function resolvePluginsConfig(config) {
	return {
...
*/
function createPluginsDomain(deps) {
	...
	*/
	const mutate = async (name, operation) => {
		try {
			...
			if (dir === void 0) return { error: noProfile() };
...
const VisionSettingsSchema = z$1.object({
	...
	harnessModels: z$1.array(z$1.object({
		provider: z$1.string(),
		model: z$1.string()
	})).default([]),
...
function staticVisionSettingsBase(config) {
	...
	if (config.visionHarnessModels !== void 0) base["harnessModels"] = config.visionHarnessModels;
...
var VisionTranscriber = class {
	...
	recordFailure(source, label, message) {
		this.failures.push({
			time: Date.now(),
			source,
			label,
	...
	*/
	async harnessCandidates() {
		...
		if (this.settings.provider !== "" && this.settings.model !== "") push(this.settings.provider, this.settings.model, HARNESS_POOL_CAP);
		...
		try {
			for (const provider of llm.listProviders()) {
				...
				try {
					...
					for (const model of models) if ((model.inputModalities ?? []).includes("image")) push(provider.id, model.id, HARNESS_CANDIDATE_CAP);
				} catch {}
			}
		} catch {}
		return list;
	...
	*/
	async endpointAttempts() {
		...
		if (baseURL !== "") {
			...
			for (const model of ordered) attempts.push({
				model,
				baseURL,
...
var VisionInterceptor = class extends Service {
	...
	async status() {
		...
		const endpointConfigured = this.settings.baseUrl.trim() !== "" && (this.settings.endpointModel.trim() !== "" || this.settings.endpointModels.length > 0);
		return {
			mounted: true,
...
//#endregion
//#region lib/types/vision-gateway.js
/**
...
*/
const visionConfigFragment = z$1.object({
	...
	visionHarnessModels: z$1.array(z$1.object({
		provider: z$1.string(),
		model: z$1.string()
	})).default([]),
	...
	return {
		...
		*/
		async configSet(request) {
			try {
				...
				return { error: errorOf(error, error.code === "SETTINGS_CONFLICT" ? "vision-config-conflict" : "vision-config-save") };
			}
		},
		/**
		...
		*/
		async endpointModels(request) {
			try {
				const saved = settingsFace(ctx)?.get(VISION_SETTINGS_NS);
				...
				const message = error instanceof Error ? error.message : String(error);
				return { error: errorOf(error, error instanceof Error && error.name === "TimeoutError" || /aborted due to timeout|timed out/iu.test(message) ? "vision-endpoint-timeout" : "vision-endpoint-fetch") };
			}
		}
	};
}
...
//#endregion
//#region lib/types/board-gateway.js
/**
...
*/
function createBoardDomain(deps) {
...
const MEMORY_SETTINGS_NS = "dsh-web-enhanced-memory";
//#endregion
//#region lib/types/global-prompt-gateway.js
/**
...
*/
function createGlobalPromptDomain(ctx) {
	return {
		async get() {
			try {
				const settings = settingsFace(ctx);
				if (settings === void 0) return { error: {
					code: "global-prompt-settings-unavailable",
					message: "the settings service is not mounted in this deployment"
				} };
				const raw = settings.get(GLOBAL_PROMPT_SETTINGS_NS);
				if (raw === void 0 || typeof raw !== "object") return { error: {
					code: "global-prompt-settings-unmanaged",
					message: `settings namespace '${GLOBAL_PROMPT_SETTINGS_NS}' is not registered`
				} };
		...
		*/
		async set(request) {
			try {
				if (typeof request.enabled !== "boolean" || typeof request.text !== "string") return { error: {
					code: "global-prompt-invalid",
					message: "enabled must be a boolean and text must be a string"
				} };
				if (request.text.length > 1e5) return { error: {
					code: "global-prompt-too-long",
					message: `text exceeds the ${String(GLOBAL_PROMPT_MAX_CHARS)}-character limit`
				} };
				const settings = settingsFace(ctx);
				if (settings === void 0) return { error: {
					code: "global-prompt-settings-unavailable",
				...
				if (!settings.writable) return { error: {
					code: "global-prompt-settings-readonly",
					message: "the settings provider is read-only"
				} };
				if (settings.get("dsh-web-enhanced-global-prompt") === void 0) return { error: {
					code: "global-prompt-settings-unmanaged",
					message: `settings namespace '${GLOBAL_PROMPT_SETTINGS_NS}' is not registered`
				} };
				...
				return {
					ok: true,
					revision: settings.describe().find((entry) => entry.ns === "dsh-web-enhanced-global-prompt")?.revision ?? 0
				};
			} catch (error) {
				return { error: errorOf(error, error.code === "SETTINGS_CONFLICT" ? "global-prompt-config-conflict" : "global-prompt-config-save") };
			}
		}
	};
}
//#endregion
//#region lib/types/memory-gateway.js
/**
...
*/
function createMemoryDomain(deps) {
	return {
		/** List memory records, optionally narrowed to one workspace. */
		async list(request) {
			try {
				const workspaceId = request.workspaceId === void 0 || request.workspaceId === null ? void 0 : deps.workspace.resolveId(request.workspaceId);
				if (request.workspaceId !== void 0 && request.workspaceId !== null && workspaceId === null) return { error: workspaceNotFound(request.workspaceId) };
				return { memories: await deps.store.list(workspaceId) };
			} catch (error) {
		...
		async remove(request) {
			try {
				...
				if (id === "") return { error: {
					code: "invalid-id",
					message: "memory id must not be empty"
				} };
				return { removed: await deps.store.delete(id) };
			} catch (error) {
		...
		async configGet() {
			try {
				...
				if (settings === void 0) return { error: {
					code: "memory-settings-unavailable",
					message: "the settings service is not mounted in this deployment"
				} };
				const raw = settings.get(MEMORY_SETTINGS_NS);
				if (raw === void 0 || typeof raw !== "object") return { error: {
					code: "memory-settings-unmanaged",
					message: `settings namespace '${MEMORY_SETTINGS_NS}' is not registered`
				} };
		...
		*/
		async configSet(request) {
			try {
				if (typeof request.enabled !== "boolean") return { error: {
					code: "memory-config-invalid",
				...
				if (!settings.writable) return { error: {
					code: "memory-settings-readonly",
					message: "the settings provider is read-only"
				} };
				if (settings.get("dsh-web-enhanced-memory") === void 0) return { error: {
					code: "memory-settings-unmanaged",
					message: `settings namespace '${MEMORY_SETTINGS_NS}' is not registered`
				} };
				...
				return {
					ok: true,
					revision: settings.describe().find((entry) => entry.ns === "dsh-web-enhanced-memory")?.revision ?? 0
				};
			} catch (error) {
				return { error: errorOf(error, error.code === "SETTINGS_CONFLICT" ? "memory-config-conflict" : "memory-config-save") };
			}
		}
	};
}
...
function memorySearchTerms(query) {
	...
	for (const raw of query.toLowerCase().split(/\s+/)) {
		...
		const chars = [...raw.replace(/[^\p{Script=Han}]/gu, "")];
		if (chars.length === 1) terms.push(chars[0]);
		else for (let index = 0; index + 1 < chars.length; index += 1) terms.push(chars[index] + chars[index + 1]);
	}
...
function scoreRecord(record, terms, workspaceId) {
	...
	let hits = 0;
	let raw = 0;
	for (const term of terms) if (summary.includes(term)) {
		hits += 1;
	...
	if (hits < requiredHits(terms.length)) return null;
	const scope = record.workspaceId !== null && record.workspaceId === workspaceId ? WORKSPACE_SCOPE_BONUS : 1;
	return {
		record,
		score: raw / terms.length * KIND_WEIGHT[record.kind] * scope
	};
}
...
var MemoryStore = class {
	...
	*/
	constructor(ctx, domain) {
		this.ready = domain !== void 0 ? domain : ctx.get("storageDomain").open(memoryDomainSpec);
	}
	/**
	...
	*/
	async save(input) {
		...
		for (const [id, record] of [...table.entries()]) if (record.workspaceId === input.workspaceId && record.summary === summary && record.updatedAt > dayAgo) {
	...
	*/
	async list(workspaceId) {
		return [...(await this.ready).table("memories").entries()].map(([, record]) => record).filter((record) => workspaceId === void 0 || record.workspaceId === workspaceId);
	}
	/**
	...
	*/
	async delete(id) {
		return await (await this.ready).table("memories").delete(id);
	}
	/**
	...
	*/
	async visibleTo(workspaceId) {
		return (await this.list()).filter((record) => record.workspaceId === null || record.workspaceId === workspaceId).reverse().sort((left, right) => right.updatedAt - left.updatedAt);
	}
	/**
	...
	*/
	async search(workspaceId, query, limit = 3) {
		...
		for (const record of records) {
			if (record.workspaceId !== null && record.workspaceId !== workspaceId) continue;
			const candidate = scoreRecord(record, terms, workspaceId);
			if (candidate !== null) scored.push(candidate);
		}
		return scored.sort((left, right) => right.score - left.score || right.record.updatedAt - left.record.updatedAt).slice(0, limit).map((candidate) => candidate.record);
	}
	/**
	...
	*/
	async enforceWorkspaceCap(domain, workspaceId) {
		const records = [...domain.table("memories").entries()].map(([, record]) => record).filter((record) => record.workspaceId === workspaceId).sort((left, right) => left.updatedAt - right.updatedAt);
...
function createServices(ctx, config) {
	...
	return {
		...
		git,
		board,
		memory: createMemoryDomain({
			ctx,
			workspace,
			store: new MemoryStore(ctx, board.domain)
		}),
		model: createModelDomain({
			ctx,
			config
		}),
		plugins: createPluginsDomain({
			ctx,
			config,
			outputMaxBytes: config.gitOutputMaxBytes
		}),
...
let WebEnhancedGateway = (() => {
	...
	return class WebEnhancedGateway extends _classSuper {
		static {
			...
			__esDecorate(this, null, _modelRouteDescribe_decorators, {
				kind: "method",
				name: "modelRouteDescribe",
				static: false,
				private: false,
				access: {
					has: (obj) => "modelRouteDescribe" in obj,
					get: (obj) => obj.modelRouteDescribe
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _deepseekRateGet_decorators, {
				kind: "method",
				name: "deepseekRateGet",
				static: false,
				private: false,
				access: {
					has: (obj) => "deepseekRateGet" in obj,
					get: (obj) => obj.deepseekRateGet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _opencodeGoUsageGet_decorators, {
				kind: "method",
				name: "opencodeGoUsageGet",
				static: false,
				private: false,
				access: {
					has: (obj) => "opencodeGoUsageGet" in obj,
					get: (obj) => obj.opencodeGoUsageGet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _modelRetryGet_decorators, {
				kind: "method",
				name: "modelRetryGet",
				static: false,
				private: false,
				access: {
					has: (obj) => "modelRetryGet" in obj,
					get: (obj) => obj.modelRetryGet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _modelRetrySet_decorators, {
				kind: "method",
				name: "modelRetrySet",
				static: false,
				private: false,
				access: {
					has: (obj) => "modelRetrySet" in obj,
					get: (obj) => obj.modelRetrySet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _globalPromptGet_decorators, {
				kind: "method",
				name: "globalPromptGet",
				static: false,
				private: false,
				access: {
					has: (obj) => "globalPromptGet" in obj,
					get: (obj) => obj.globalPromptGet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _globalPromptSet_decorators, {
				kind: "method",
				name: "globalPromptSet",
				static: false,
				private: false,
				access: {
					has: (obj) => "globalPromptSet" in obj,
					get: (obj) => obj.globalPromptSet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _memoryDelete_decorators, {
				kind: "method",
				name: "memoryDelete",
				static: false,
				private: false,
				access: {
					has: (obj) => "memoryDelete" in obj,
					get: (obj) => obj.memoryDelete
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _memoryConfigGet_decorators, {
				kind: "method",
				name: "memoryConfigGet",
				static: false,
				private: false,
				access: {
					has: (obj) => "memoryConfigGet" in obj,
					get: (obj) => obj.memoryConfigGet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _memoryConfigSet_decorators, {
				kind: "method",
				name: "memoryConfigSet",
				static: false,
				private: false,
				access: {
					has: (obj) => "memoryConfigSet" in obj,
					get: (obj) => obj.memoryConfigSet
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _gitCommitDiff_decorators, {
				kind: "method",
				name: "gitCommitDiff",
				static: false,
				private: false,
				access: {
					has: (obj) => "gitCommitDiff" in obj,
					get: (obj) => obj.gitCommitDiff
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		constructor(ctx, config = {}) {
			...
			ctx.on("llm/adapters-updated", () => {
				this.services.model.clearRouteNames();
			});
		}
		/** List every task, oldest first. */
		taskList() {
...
//#endregion
//#region lib/types/global-prompt.js
/**
...
const GLOBAL_PROMPT_SECTION = "web-enhanced:global-prompt";
/**
...
*/
const GLOBAL_PROMPT_ORDER = 50;
/** Schema of the `dsh-web-enhanced-global-prompt` settings namespace. */
const GlobalPromptSettingsSchema = z$1.object({
...
*/
function globalPromptTextOf(value) {
...
*/
function applyGlobalPrompt(ctx) {
...
const MEMORY_SECTION = "web-enhanced:memory";
/**
...
const MemorySettingsSchema = z$1.object({ enabled: z$1.boolean().default(true) });
...
let standingIds = /* @__PURE__ */ new Set();
let store;
let settingsScope;
let registry;
/**
...
function memoryEnabled() {
	if (settingsScope === void 0) return true;
...
function resolveWorkspaceId(registry, cwd) {
	if (registry === void 0 || cwd === null) return null;
...
const memoryToolDefinition = {
	...
	output: {
		...
		render(_args, value) {
			return [{
				type: "text",
				text: value.deduplicated === true ? `更新了已有记忆 ${String(value.id)}。` : `已保存记忆 ${String(value.id)}。`
			}];
		}
	},
	async execute(args, exec) {
		...
		const sourceSessionId = session?.id === void 0 ? null : String(session.id);
...
*/
function applyMemory(ctx, domain) {
	standingText = "";
	standingIds = /* @__PURE__ */ new Set();
	...
	if (settings !== void 0 && typeof settings.register === "function") try {
	...
	const systemPrompt = ctx.get("systemPrompt", false);
	if (systemPrompt !== void 0 && typeof systemPrompt.section === "function") ctx.effect(() => systemPrompt.section({
		name: MEMORY_SECTION,
		order: 60,
		text: () => memoryEnabled() ? standingText : ""
	}), "dsh-web-enhanced: memory prompt section");
	const tools = ctx.get("tools", false);
	if (tools !== void 0 && typeof tools.register === "function") ctx.effect(() => tools.register(memoryToolDefinition), "dsh-web-enhanced: save_memory tool");
	const injectedContent = /* @__PURE__ */ new Map();
	ctx.on("agent/pre-step", (async (payload, next) => {
		const decision = await next();
		if (decision === null || decision === void 0) return decision;
		if (decision.kind !== "enter") return decision;
		if (!memoryEnabled()) return decision;
		const session = payload.agent?.session;
		if (session === void 0 || store === void 0) return decision;
		const sessionKey = session.id === void 0 ? "" : String(session.id);
		const cwd = typeof session.header?.cwd === "string" ? session.header.cwd : null;
		const workspaceId = resolveWorkspaceId(registry, cwd);
		await updateStanding(workspaceId);
		const query = lastUserText(payload.messages ?? []) || lastUserQuery(session);
		if (query === "") return decision;
		const hits = (await store.search(workspaceId, query)).filter((hit) => !standingIds.has(String(hit.id)));
		...
		if (injectedContent.get(sessionKey) === content) return decision;
...
//#endregion
export { Config, GLOBAL_PROMPT_ORDER, GLOBAL_PROMPT_SECTION, GlobalPromptSettingsSchema, MEMORY_ORDER, MEMORY_SECTION, MEMORY_SETTINGS_NS, MemorySettingsSchema, VISION_SETTINGS_NS, VisionInterceptor, VisionSettingsSchema, VisionTranscriber, WebEnhancedGateway, apply, applyGlobalPrompt, applyMemory, globalPromptTextOf, inject, name };
