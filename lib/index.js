...
const terminalStatusSchema = z.union([z.object({ kind: z.literal("running") }), z.object({
...
const WEB_ENHANCED_DESCRIPTORS = [
	...
	unary("terminalList", "TerminalListRequest", "TerminalListResult", okOrError(z.object({ sessions: z.array(terminalSessionSchema) }))),
...
function statusView(status) {
	return status.kind === "running" ? { kind: "running" } : {
		kind: "exited",
...
var TerminalHost = class {
	...
	ownerOf(ownerSessionId) {
		return this.ctx.get("agents", false)?.get(ownerSessionId);
	}
	/**
	...
	*/
	async open(request, cwd) {
		...
		if (terminals === void 0) return { error: {
			code: "terminal-unavailable",
			message: "the host composes no terminal service (@deepseek-ai/dsh-terminal)"
		} };
		const owner = this.ownerOf(request.ownerSessionId);
		if (owner === void 0) return { error: {
			code: "owner-not-live",
			message: `no live agent for session '${request.ownerSessionId}'`
		} };
		const backends = terminals.listBackends();
		if (backends.length === 0) return { error: {
			code: "terminal-no-backend",
			message: "no PTY backend is registered (load a terminal provider such as dsh-terminal-bash)"
		} };
		...
		try {
			...
			return {
				session: {
					sessionId,
					type: spawned.type,
					...spawned.name !== void 0 ? { name: spawned.name } : {},
					...spawned.pid !== void 0 ? { pid: spawned.pid } : {},
					status: statusView(spawned.status)
				},
				motd
			};
		} catch (error) {
	...
	*/
	async send(request) {
		...
		try {
			const result = await terminals.startSend(owner, id, {
				text: request.text,
	...
	read(request) {
		...
		try {
			const page = terminals.read(owner, id, {
				...
				...request.count !== void 0 ? { count: request.count } : {}
			});
			return {
				text: page.text,
	...
	*/
	async signal(request) {
		if (!SIGNALS.includes(request.signal)) return { error: {
			code: "terminal-signal",
			message: `unsupported signal '${request.signal}'`
		} };
		const boundaries = this.owned(request.ownerSessionId, request.sessionId);
		if ("error" in boundaries) return boundaries;
		try {
			...
			return {
				...
				targetPgid: delivered.targetPgid
			};
		} catch (error) {
			return { error: {
				code: "terminal-signal",
	...
	*/
	async list(ownerSessionId) {
		...
		if (owner === void 0) return { error: {
			code: "owner-not-live",
			message: `no live agent for session '${ownerSessionId}'`
		} };
		return { sessions: terminals.list(owner).map((session) => ({
			sessionId: session.sessionId,
			type: session.type,
			...session.name !== void 0 ? { name: session.name } : {},
			...session.pid !== void 0 ? { pid: session.pid } : {},
		...
		*/
		...
		async terminalOpen(request) {
			...
			if (root === null) return { error: {
				code: "workspace-not-found",
				message: `workspace '${String(request.workspaceId)}' does not exist`
			} };
