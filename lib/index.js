...
const WEB_ENHANCED_DESCRIPTORS = [
	...
	nullary("modelRetryGet", "ModelRetryGetResult", okOrError(z.object({ config: z.object({
		...
		*/
		async modelRetryGet() {
			try {
				...
				const raw = settings.get(MODEL_RETRY_SETTINGS_NS);
				if (raw === void 0 || typeof raw !== "object") return { error: {
					code: "model-retry-settings-unmanaged",
					message: `settings namespace '${MODEL_RETRY_SETTINGS_NS}' is not registered`
				} };
				...
				return { config: {
					...
					maxDelayMs: policy?.backoff?.maxDelayMs ?? 1e4,
					jitterRatio: policy?.backoff?.jitterRatio ?? .1
				} };
			} catch (error) {
		...
		async modelRetrySet(request) {
			try {
				...
				if (settings.get(MODEL_RETRY_SETTINGS_NS) === void 0) return { error: {
					code: "model-retry-settings-unmanaged",
					message: `settings namespace '${MODEL_RETRY_SETTINGS_NS}' is not registered`
				} };
				await settings.update(MODEL_RETRY_SETTINGS_NS, { retryPolicy: {
					mode: "normal",
					maxRetries: request.maxRetries
				} }, request.expectedRevision);
				return {
					ok: true,
					revision: settings.describe().find((entry) => entry.ns === MODEL_RETRY_SETTINGS_NS)?.revision ?? 0
				};
			} catch (error) {
				const conflict = error.code === "SETTINGS_CONFLICT";
