...
var MemoryStore = class {
	...
	*/
	async save(input) {
		...
		for (const [id, record] of [...table.entries()]) if (record.workspaceId === input.workspaceId && record.summary === input.summary && record.updatedAt > dayAgo) {
	...
	*/
	async byWorkspace(workspaceId) {
		return [...await this.list(workspaceId)].sort((left, right) => right.updatedAt - left.updatedAt);
	}
	/**
	...
	*/
	async search(workspaceId, query) {
		...
		return [...await this.list(workspaceId)].map((record) => {
			const haystack = `${record.summary} ${record.body}`.toLowerCase();
			return {
				record,
				hits: terms.filter((term) => haystack.includes(term)).length
			};
		}).filter((candidate) => candidate.hits > 0).sort((left, right) => right.hits - left.hits).slice(0, 3).map((candidate) => candidate.record);
	}
	/**
...
function resolveWorkspaceId(registry, cwd) {
	...
	try {
		...
		for (const workspace of workspaces) if (workspace.path === cwd) return workspace.id;
	} catch {
		return null;
	}
	return null;
...
function applyMemory(ctx, domain) {
	...
	ctx.on("agent/pre-step", (async (payload, next) => {
		...
		if (typeof agent?.inject === "function") try {
