...
const WEB_ENHANCED_DESCRIPTORS = [
	...
	unary("terminalList", "TerminalListRequest", "TerminalListResult", okOrError(z.object({ terminals: z.array(terminalViewSchema) }))),
	unary("terminalSpawn", "TerminalSpawnRequest", "TerminalSpawnResult", okOrError(z.object({ terminal: terminalViewSchema }))),
	unary("terminalClose", "TerminalCloseRequest", "TerminalCloseResult", okOrError(z.object({ closed: z.boolean() }))),
	unary("terminalSignal", "TerminalSignalRequest", "TerminalSignalResult", okOrError(z.object({ delivered: z.boolean() })))
];
//#endregion
...
var Scrollback = class {
	...
	*/
	append(text) {
		...
		while (this.bytes > this.maxBytes && this.chunks.length > 1) this.bytes -= Buffer.byteLength(this.chunks.shift());
	}
	/**
...
var TerminalRegistry = class {
	...
	*/
	async spawn(workspaceId, cwd, cols, rows) {
		...
		handle.done.then((outcome) => {
			this.settle(record, outcome.exitCode, outcome.signal);
		}, () => {
	...
	*/
	async write(terminalId, data) {
		const record = this.records.get(terminalId);
		if (record === void 0 || !record.running) return false;
	...
	*/
	list(workspaceId) {
		return [...this.records.values()].filter((record) => record.workspaceId === workspaceId).map((record) => this.viewOf(record));
	}
...
*/
function createTerminalRegistry(ctx, subprocess, limits) {
	const registry = new TerminalRegistry(subprocess, limits);
	ctx.effect(() => () => {
		registry.disposeAll();
	}, "web-enhanced: terminal sessions");
	return registry;
}
//#endregion
...
*/
const TERMINAL_SOCKET_PATH = "/plugins/dsh-web-enhanced/terminal";
/** Close code telling the browser the shell exited, so it must not reconnect. */
const TERMINAL_EXIT_CLOSE_CODE = 4e3;
/** Close code for a refused attach (unknown session), so it must not reconnect. */
const TERMINAL_GONE_CLOSE_CODE = 4001;
//#endregion
//#region lib/types/terminal-socket.js
/**
...
*/
function isTrustedTerminalUpgrade(req, trustedHosts) {
	const host = req.headers.host;
	if (host === void 0) return false;
	if (!isLoopbackAuthority(host) && !trustedHosts.includes(host)) return false;
...
*/
function applyTerminalSocket(ctx, registry, options) {
	ctx.inject(["webServer"], (webCtx) => {
		...
		webCtx.effect(() => () => {
			sockets.close();
		}, "web-enhanced: terminal socket server");
...
function attach(connection, registry, terminalId) {
	const detach = registry.attach(terminalId, {
		send(data) {
			if (connection.readyState === connection.OPEN) connection.send(data);
		},
		exit(exitCode, signal) {
			if (connection.readyState !== connection.OPEN) return;
			connection.close(TERMINAL_EXIT_CLOSE_CODE, JSON.stringify({
				exitCode,
				signal
			}));
		}
	...
	connection.on("message", (data) => {
		registry.write(terminalId, Buffer.isBuffer(data) ? data.toString("utf8") : String(data));
	});
...
const Config = z$1.object({
	...
	terminalShell: z$1.string().default(""),
	terminalScrollbackBytes: z$1.number().default(262144),
	terminalGraceMs: z$1.number().default(5e3),
	terminalTrustedHosts: z$1.array(z$1.string()).default([]),
...
function resolveConfig(config) {
	return {
		...
		terminalScrollbackBytes: config.terminalScrollbackBytes ?? 262144,
		terminalGraceMs: config.terminalGraceMs ?? 5e3,
...
let WebEnhancedGateway = (() => {
	...
	return class WebEnhancedGateway extends _classSuper {
		static {
			...
			_terminalSpawn_decorators = [Remote("terminalSpawn")];
			...
			__esDecorate(this, null, _terminalList_decorators, {
				kind: "method",
				name: "terminalList",
				static: false,
				private: false,
				access: {
					has: (obj) => "terminalList" in obj,
					get: (obj) => obj.terminalList
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _terminalSpawn_decorators, {
				kind: "method",
				name: "terminalSpawn",
				static: false,
				private: false,
				access: {
					has: (obj) => "terminalSpawn" in obj,
					get: (obj) => obj.terminalSpawn
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		emptySearchCache = /* @__PURE__ */ new Map();
		/**
		...
		*/
		constructor(ctx, config = {}) {
			...
			this.routeNames = new ModelRouteNames(ctx.get("llm", false));
		...
		async modelRouteDescribe(request) {
			try {
		...
		*/
		...
		async memoryList(request) {
			try {
				const workspaceId = request.workspaceId === void 0 || request.workspaceId === null ? void 0 : this.resolveWorkspaceId(request.workspaceId);
				if (request.workspaceId !== void 0 && request.workspaceId !== null && workspaceId === null) return { error: {
					code: "workspace-not-found",
					message: `workspace '${request.workspaceId}' does not exist`
				} };
				return { memories: await this.memoryStore.list(workspaceId) };
			} catch (error) {
				return { error: this.errorOf(error, "memory-list") };
			}
		}
		/** Delete one memory record by id. */
		async memoryDelete(request) {
			try {
				const id = request.id;
				if (id === "") return { error: {
					code: "invalid-id",
				...
				return { removed: await this.memoryStore.delete(id) };
			} catch (error) {
		...
		*/
		...
		async gitCommitDiff(request) {
			return this.withGit(request.workspaceId, async (client) => ({ text: await client.commitDiff(request.hash, request.path) }));
		}
		/**
		...
		*/
		async fsSearch(request) {
			...
			try {
				const result = { entries: await searchFiles(root, rel, request.query ?? "", this.fsLimits) };
		...
		*/
		async terminalSpawn(request) {
			...
			try {
				return { terminal: await this.terminals.spawn(request.workspaceId, root, request.cols, request.rows) };
			} catch (error) {
		...
		*/
		storeEmptySearch(key, result) {
			...
			this.emptySearchCache.set(key, {
				at: Date.now(),
				result
			});
			while (this.emptySearchCache.size > FS_SEARCH_CACHE_LIMIT) this.emptySearchCache.delete(this.emptySearchCache.keys().next().value);
		}
		/**
		...
		*/
		invalidateSearchCache(workspaceId) {
			const prefix = `${workspaceId}:`;
			for (const key of [...this.emptySearchCache.keys()]) if (key.startsWith(prefix)) this.emptySearchCache.delete(key);
		}
		get fsLimits() {
			return {
