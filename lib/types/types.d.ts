...
export type DeepSeekRateGetResult = DeepSeekRateView | {
    readonly error: ApiError;
};
...
export type ModelRetryGetResult = {
    readonly configs: readonly ModelRetryConfigView[];
} | {
    readonly error: ApiError;
};
...
export type ModelRetrySetResult = ModelRetrySetView | {
    readonly error: ApiError;
};
...
export type VisionEndpointModelsResult = VisionEndpointModelsView | {
    readonly error: ApiError;
};
/** Settings namespace owning the user-editable global system prompt. */
export declare const GLOBAL_PROMPT_SETTINGS_NS: "dsh-web-enhanced-global-prompt";
/** Upper bound on the global prompt text the settings page accepts. */
export declare const GLOBAL_PROMPT_MAX_CHARS: 100000;
...
export type GlobalPromptGetResult = GlobalPromptConfigView | {
    readonly error: ApiError;
};
...
export type GlobalPromptSetResult = GlobalPromptSaveView | {
    readonly error: ApiError;
};
...
export type MemoryConfigGetResult = MemoryConfigView | {
    readonly error: ApiError;
};
 ...
 */
export declare const TERMINAL_SOCKET_PATH: "/plugins/dsh-web-enhanced/terminal";
/** Close code telling the browser the shell exited, so it must not reconnect. */
export declare const TERMINAL_EXIT_CLOSE_CODE: 4000;
/** Close code for a refused attach (unknown session), so it must not reconnect. */
export declare const TERMINAL_GONE_CLOSE_CODE: 4001;
/** Identifies one live terminal session held by this plugin's registry. */
export type TerminalId = Branded<'TerminalId'>;
/**
 * Signals the terminal surface delivers to a session's foreground process
 * group. Member-identical to the subprocess seam's `SubprocessTerminalSignal`;
 * change both together.
 */
export type TerminalSignalName = 'SIGINT' | 'SIGTERM' | 'SIGKILL' | 'SIGTSTP' | 'SIGHUP';
/** Client-visible summary of one terminal session. */
export interface TerminalView {
    readonly id: string;
    readonly workspaceId: string;
    /** Display label; defaults to the spawned shell's base name. */
    readonly title: string;
    /** Top-level shell process id. */
    readonly pid: number;
    /** False once the shell exited; the record survives until closed or reaped. */
    readonly running: boolean;
    readonly createdAt: number;
    /**
     * Terminal geometry fixed at spawn. Re-attaching browsers must render at
     * exactly these dimensions: the PTY cannot be resized, so a viewport sized
     * to the window instead would wrap every line at the wrong column.
     */
...
/**
 * One terminal creation. `cols`/`rows` are measured by the browser at open
 * time and fixed for the session's life — the subprocess seam allocates the
 * terminal at spawn and exposes no resize.
 */
...
/** Terminal close Remote result; `closed` is false when the id was unknown. */
export type TerminalCloseResult = {
    readonly closed: boolean;
} | {
    readonly error: ApiError;
};
/** One signal delivery to a session's foreground process group. */
export interface TerminalSignalRequest {
    readonly terminalId: string;
    readonly signal: TerminalSignalName;
}
/** Terminal signal Remote result. */
