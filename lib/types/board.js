...
const sharedMigrations = new WeakMap();
/**
 ...
 */
export async function migrateJsonDomainV1ToV2(ctx) {
    const facility = ctx.get('storageDomain', false);
    const hub = ctx.get('storage', false);
    if (facility === undefined || hub === undefined)
        return;
    ...
    if (typeof unit !== 'object' || unit === null)
        return;
    const header = unit;
    if (header['name'] !== 'web_enhanced' || header['version'] !== 1)
        return;
    ...
    if (tableRecord['memories'] === undefined)
        tableRecord['memories'] = {};
    try {
 ...
 */
export async function openSharedDomain(ctx) {
...
export function resolveBoardConfig(config) {
