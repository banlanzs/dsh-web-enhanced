...
export function staticVisionSettingsBase(config) {
    ...
    if (config.visionHarnessModels !== undefined)
        base['harnessModels'] = config.visionHarnessModels;
    ...
    if (config.visionEndpointModels !== undefined)
        base['endpointModels'] = config.visionEndpointModels;
...
export class VisionTranscriber {
    ...
    recordFailure(source, label, message) {
        ...
        if (this.failures.length > ATTEMPT_FAILURE_CAP)
            this.failures.splice(0, this.failures.length - ATTEMPT_FAILURE_CAP);
    }
    ...
    async harnessCandidates() {
        ...
        const push = (provider, model, cap) => {
            if (provider.trim() === '' || model.trim() === '' || list.length >= cap)
                return;
        ...
        if (this.settings.harnessModels.length > 0) {
            for (const entry of this.settings.harnessModels)
                push(entry.provider, entry.model, HARNESS_POOL_CAP);
            return list;
        }
                ...
                try {
                    ...
                    for (const model of models) {
                        if ((model.inputModalities ?? []).includes('image'))
                            push(provider.id, model.id, HARNESS_CANDIDATE_CAP);
                    }
                }
                catch {
    ...
    async streamHarness(candidate, ref, signal) {
        ...
        if (llm === undefined || typeof llm.stream !== 'function')
            return { text: null, error: 'llm service unavailable' };
        const messages = [{
    ...
    async endpointAttempts() {
        ...
        if (baseURL !== '') {
            const ordered = [];
            if (this.settings.endpointModel.trim() !== '')
                ordered.push(this.settings.endpointModel.trim());
        ...
        for (const fallback of this.settings.fallbacks) {
            ...
            if (fallback.model.trim() === '' || fallbackURL.trim() === '')
                continue;
            attempts.push({
                model: fallback.model,
