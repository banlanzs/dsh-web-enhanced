...
export function officeKindOf(name) {
 ...
 */
export async function officePreviewView(root, rel, limits) {
...
function* scanXml(xml) {
    ...
    while (index < length) {
        ...
        if (open === -1) {
            if (index < length)
                yield xml.slice(index);
            return;
        }
        if (open > index)
            yield xml.slice(index, open);
    ...
    for (const event of scanXml(xml)) {
        if (typeof event === 'string') {
            ...
            if (current.children.length === 0)
                ...
                current.children[current.children.length - 1].text += event;
            continue;
        }
        if (event.closing) {
            if (stack.length > 1)
        ...
        const node = { name: event.name, attrs: event.attrs, children: [], text: '' };
        const parent = stack[stack.length - 1];
        parent.children.push(node);
        if (!event.selfClosing && stack.length < MAX_XML_DEPTH)
            stack.push(node);
    }
    return root;
}
...
function collectText(node) {
    if (node.name === 'w:t' || node.name === 't')
        return node.text;
    if (node.name === 'w:tab' || node.name === 'tab')
        return '\t';
    if (node.name === 'w:br' || node.name === 'br')
        return '\n';
    // Field codes, deleted revisions, and phonetic guides are not content.
    if (node.name === 'w:instrText' || node.name === 'w:delText' || node.name === 'rPh')
        return '';
    let out = '';
...
function extractTable(table) {
    ...
    for (const row of descendants(table, 'w:tr')) {
        ...
        for (const cell of descendants(row, 'w:tc')) {
            if (cells.length >= OFFICE_MAX_TABLE_COLS)
                break;
    ...
    const walk = (node) => {
        ...
        if (node.name === 'w:tbl') {
            ...
            if (table.rows.length > 0)
                push({ type: 'table', rows: table.rows });
            if (table.truncated)
                truncated = true;
            return;
        }
        if (node.name === 'w:sectPr' || node.name === 'w:bookmarkStart' || node.name === 'w:bookmarkEnd')
            return;
        for (const child of node.children)
            walk(child);
    };
...
function convertXlsx(data) {
    ...
    if (sheetXml === undefined)
        throw new Error('xlsx: no worksheet found');
    ...
    for (const row of descendants(root, 'row')) {
        ...
        for (const cell of descendants(row, 'c')) {
            ...
            while (cells.length < index)
                cells.push('');
            if (index < OFFICE_MAX_TABLE_COLS)
                cells[index] = value.replace(/\s+/gu, ' ').trim();
        }
        if (cells.some(cell => cell !== ''))
            rows.push(cells);
    }
...
function firstWorksheet(files) {
    ...
    if (files['xl/worksheets/sheet1.xml'] !== undefined)
