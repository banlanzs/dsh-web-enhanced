...
export function SettingsSection({ remote, t, skin, notifications }) {
    ...
    return (_jsxs("div", { className: css.root, children: [_jsx("div", { className: css.tabs, role: "tablist", children: tabs.map(entry => (_jsx("button", { type: "button", role: "tab", "aria-selected": tab === entry.id, className: tab === entry.id ? css.tabActive : css.tab, onClick: () => { setTab(entry.id); }, children: entry.label }, entry.id))) }), _jsxs("div", { className: css.body, children: [tab === 'plugins' && _jsx(PluginManager, { remote: remote, t: t }), tab === 'general' && _jsx(GeneralSettingsPanel, { remote: remote, t: t }), tab === 'globalPrompt' && _jsx(GlobalPromptPanel, { remote: remote, t: t }), tab === 'memory' && _jsx(MemoryPanel, { remote: remote, t: t }), tab === 'vision' && _jsx(VisionStatusPanel, { remote: remote, t: t }), tab === 'notify' && _jsx(NotificationPanel, { notifications: notifications, t: t }), tab === 'skins' && _jsx(SkinPanel, { skin: skin, t: t }), tab === 'about' && _jsx(AboutPanel, { t: t })] })] }));
}
