    ...
    return blocks.map((block, index) => {
        switch (block.type) {
            ...
            case 'list': {
                ...
                return (_jsx(Tag, { "data-ordered": block.ordered ? 'true' : 'false', children: block.items.map((item, itemIndex) => (_jsxs("li", { "data-task": item.task === true ? 'true' : undefined, "data-checked": item.task === true && item.checked ? 'true' : undefined, children: [item.task === true
                                ? (_jsxs("label", { className: css.task, children: [_jsx("input", { className: css.taskBox, type: "checkbox", disabled: true, checked: item.checked }), _jsx("span", { className: css.taskText, children: _jsx(Spans, { spans: item.spans }) })] }))
                                : _jsx(Spans, { spans: item.spans }), item.children.length > 0 && _jsx(Blocks, { blocks: item.children })] }, itemIndex))) }, index));
            }
        }
    });
}
...
function MarkdownView({ source }) {
    return (_jsx("div", { className: css.markdown, children: _jsx(Blocks, { blocks: parseMarkdown(source) }) }));
}
