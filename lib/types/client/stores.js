    ...
    if (typeof record['width'] === 'object' && record['width'] !== null) {
        for (const [key, value] of Object.entries(record['width'])) {
            if (typeof value === 'number')
                width[key] = clampPanelWidth(value);
        }
    }
...
export function createPanel() {
    const cell = createCell({ tab: 'files', collapsed: {}, width: {}, expanded: {}, query: '' }, { key: PANEL_PERSIST_KEY, revive: revivePanel });
    return {
        cell,
        actions: {
 ...
 */
export function panelWidthOf(state, workspaceId) {
 ...
 */
export function panelCollapsedOf(state, workspaceId) {
    if (workspaceId === undefined)
        return false;
