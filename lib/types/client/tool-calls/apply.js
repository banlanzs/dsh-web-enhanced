        ...
        for (const run of activityRuns(items)) {
            ...
            if (!expanded) {
                for (const el of targets)
                    desiredHidden.add(el);
            }
        }
        // Reconcile the whole flow so rows unhidden by a changed answer
        // classification do not retain a stale data-we-tool-hidden attribute.
        for (const el of flow.querySelectorAll(`:scope > [${HIDDEN}]`)) {
            if (!desiredHidden.has(el))
                setAttr(el, HIDDEN, null);
        }
        for (const el of desiredHidden)
            setAttr(el, HIDDEN, '');
