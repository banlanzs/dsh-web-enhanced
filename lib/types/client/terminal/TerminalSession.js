...
export function TerminalSession({ workspaceId, terminal, remote, onSpawned, onExit, labels, }) {
    const host = useRef(null);
    const [phase, setPhase] = useState({ kind: 'connecting' });
    ...
    useEffect(() => {
        ...
        const connect = (id) => {
            ...
            connection.onmessage = (event) => { term.write(event.data); };
            connection.onclose = (event) => {
                ...
                if (event.code === TERMINAL_EXIT_CLOSE_CODE) {
                    let code = '';
                    try {
                        const settled = JSON.parse(event.reason);
                ...
                retry = setTimeout(() => { connect(id); }, Math.min(RECONNECT_MAX_MS, RECONNECT_BASE_MS * 2 ** attempt++));
            };
        };
        const start = async () => {
            if (terminalId !== null) {
    ...
    return (_jsxs("div", { className: css.session, "data-testid": "terminal-session", children: [_jsx("div", { className: css.surface, ref: host, "data-testid": "terminal-surface" }), phase.kind !== 'open' && (_jsxs("p", { className: css.status, "data-testid": "terminal-status", onClick: retryNow, children: [phase.kind === 'connecting' && labels.connecting, phase.kind === 'reconnecting' && labels.reconnecting, phase.kind === 'gone' && labels.gone, phase.kind === 'exited' && labels.exited(phase.code), phase.kind === 'error' && labels.error(phase.message)] }))] }));
}
