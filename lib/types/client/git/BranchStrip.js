 ...
 */
export function cachedGitBranches(workspaceId, fetch) {
    const hit = branchesCache.get(workspaceId);
    if (hit !== undefined && Date.now() - hit.at < BRANCHES_TTL_MS)
        return hit.promise;
    const promise = fetch();
 ...
 */
export function invalidateBranchesCache(workspaceId) {
    branchesCache.delete(workspaceId);
}
