    ...
    return (_jsx(OverlayShell, { title: t('graph.title'), closeLabel: t('graph.close'), onClose: closeOverlay, testId: "graph-overlay", actions: workspaceId === undefined
            ...
            : commits.phase === 'loading'
                ...
                : commits.phase === 'error'
                    ...
                    : (_jsx(GraphBody, { commits: commits.items, empty: t('graph.empty'), expanded: expanded, workspaceId: workspaceId, remote: remote, onToggle: hash => { setExpanded(current => (current === hash ? null : hash)); }, t: t })) }));
}
