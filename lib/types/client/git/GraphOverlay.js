...
export function GraphPanel({ workspaceId, remote, t, openTab, selectTab }) {
    ...
    return (_jsx("div", { className: css.panel, "data-testid": "graph-panel", children: workspaceId === undefined
            ...
            : (_jsxs(_Fragment, { children: [_jsxs("div", { className: css.toolbar, children: [_jsxs("label", { className: css.filter, children: [_jsx("span", { className: css.filterLabel, children: t('graph.filter') }), _jsxs("select", { className: css.select, value: branch, "data-testid": "graph-branch-filter", onChange: event => { setBranch(event.target.value); }, children: [_jsx("option", { value: ALL_BRANCHES, children: t('graph.allBranches') }), branches.map(item => (_jsx("option", { value: item.name, children: item.name }, item.name)))] })] }), _jsx("button", { type: "button", className: css.action, onClick: () => { void load(); }, children: t('graph.refresh') })] }), commits.phase === 'loading'
                        ...
                        : commits.phase === 'error'
                            ...
                            : (_jsx(GraphBody, { commits: commits.items, working: working, empty: t('graph.empty'), expanded: expanded, workingOpen: workingOpen, workspaceId: workspaceId, remote: remote, openTab: openTab, selectTab: selectTab, onToggle: hash => { setExpanded(current => (current === hash ? null : hash)); }, onToggleWorking: () => { setWorkingOpen(value => !value); }, t: t }))] })) }));
}
/** The git graph overlay: the same panel under the full-frame chrome. */
export function GraphOverlay({ useOverlay, useSessions, useWorkspaces, remote, closeOverlay, t, openTab, selectTab, }) {
    ...
    if (!open)
        return null;
    return (_jsx(OverlayShell, { title: t('graph.title'), closeLabel: t('graph.close'), onClose: closeOverlay, testId: "graph-overlay", fill: true, children: _jsx(GraphPanel, { workspaceId: workspaceId === undefined ? undefined : String(workspaceId), remote: remote, t: t, openTab: openTab, selectTab: selectTab }) }));
}
...
function GraphBody({ commits, working, empty, expanded, workingOpen, workspaceId, remote, openTab, selectTab, onToggle, onToggleWorking, t, }) {
    ...
    const workingRow = dirty
        ? (_jsx(WorkingRow, { working: working, lane: placement?.lane ?? 0, through: placement?.through ?? [], railWidth: railWidth, open: workingOpen, onToggle: onToggleWorking, workspaceId: workspaceId, remote: remote, openTab: openTab, selectTab: selectTab, t: t }))
        : null;
...
function WorkingRow({ working, lane, through, railWidth, open, onToggle, workspaceId, remote, openTab, selectTab, t, }) {
    ...
    return (_jsxs("li", { className: css.entry, children: [_jsxs("button", { type: "button", className: css.row, "aria-expanded": open, "data-testid": "graph-working-row", onClick: onToggle, children: [_jsxs("svg", { className: css.rail, width: railWidth, height: ROW_HEIGHT, "aria-hidden": true, children: [through.map(other => (_jsx("line", { className: css.edge, "data-lane": laneColor(other, PALETTE_SIZE), x1: (other + 1) * LANE_STEP, y1: 0, x2: (other + 1) * LANE_STEP, y2: ROW_HEIGHT }, `through-${String(other)}`))), _jsx("line", { className: css.pendingEdge, "data-lane": laneColor(lane, PALETTE_SIZE), x1: dotX, y1: ROW_HEIGHT / 2, x2: dotX, y2: ROW_HEIGHT }), _jsx("circle", { className: css.pendingDot, "data-lane": laneColor(lane, PALETTE_SIZE), cx: dotX, cy: ROW_HEIGHT / 2, r: 4 })] }), _jsx("span", { className: css.hash, children: "\u2022\u2022\u2022\u2022\u2022\u2022" }), _jsx("span", { className: css.subject, children: t('graph.working.title') }), _jsx("span", { className: css.workingCounts, children: t('graph.working.counts', {
                        ...
                        }) })] }), open && (_jsx(WorkingDetail, { working: working, workspaceId: workspaceId, remote: remote, openTab: openTab, selectTab: selectTab, t: t }))] }));
}
...
function WorkingDetail({ working, workspaceId, remote, openTab, selectTab, t, }) {
    return (_jsxs("div", { className: css.detail, "data-testid": "graph-working-detail", children: [working.truncated && (_jsx("p", { className: css.filesTitle, children: t('graph.working.truncated', { count: String(working.files.length) }) })), _jsx("ul", { className: css.files, children: working.files.map(file => (_jsx("li", { className: css.file, children: _jsxs("button", { type: "button", className: css.fileButton, title: `${file.path} — ${t('graph.diffHint')}`, onClick: () => { void openWorkingDiff(workspaceId, file, remote, openTab, selectTab); }, children: [_jsx("span", { className: css.stateTag, "data-state": file.state, children: stateLabel(file, t) }), _jsx("span", { className: css.filePath, children: file.path }), _jsx("span", { className: css.added, title: file.added === null ? t('graph.working.unknown') : undefined, children: file.added === null ? '—' : `+${String(file.added)}` }), _jsx("span", { className: css.removed, children: file.removed === null ? '—' : `-${String(file.removed)}` })] }) }, `${file.state}:${file.path}`))) })] }));
}
...
function CommitDetail({ hash, workspaceId, remote, openTab, selectTab, t }) {
    ...
    return (_jsxs("div", { className: css.detail, "data-testid": "graph-detail", children: [_jsxs("dl", { className: css.facts, children: [_jsx("dt", { children: t('graph.detail.hash') }), _jsx("dd", { className: css.mono, children: commit.hash }), _jsx("dt", { children: t('graph.detail.parents') }), _jsx("dd", { className: css.mono, children: commit.parents.length === 0 ? '—' : commit.parents.map(shortHash).join(' ') }), _jsx("dt", { children: t('graph.detail.author') }), _jsx("dd", { children: commit.email === '' ? commit.author : `${commit.author} <${commit.email}>` }), _jsx("dt", { children: t('graph.detail.date') }), _jsx("dd", { children: new Date(commit.date * 1000).toLocaleString() })] }), commit.body !== '' && _jsx("pre", { className: css.body, children: commit.body }), _jsx("p", { className: css.filesTitle, children: t('graph.detail.files', { count: String(commit.files.length) }) }), commit.files.length > 0 && (_jsx("ul", { className: css.files, children: commit.files.map(file => (_jsx("li", { className: css.file, children: _jsxs("button", { type: "button", className: css.fileButton, title: `${file.path} — ${t('graph.diffHint')}`, onClick: () => {
                        ...
                        }, children: [_jsx("span", { className: css.filePath, children: file.path }), _jsx("span", { className: css.added, children: file.added === null ? '—' : `+${String(file.added)}` }), _jsx("span", { className: css.removed, children: file.removed === null ? '—' : `-${String(file.removed)}` })] }) }, file.path))) }))] }));
}
