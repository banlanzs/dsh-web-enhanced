 ...
 */
export function previewKindOf(path) {
    ...
    if (ext === '' && (name === 'dockerfile' || name === 'makefile'))
        return 'code';
    return 'text';
}
/**
 ...
 */
export function hasRenderedForm(kind) {
 ...
 */
export async function loadPreviewTab(remote, workspaceId, path) {
    const kind = previewKindOf(path);
    const base = { path, name: baseNameOf(path), kind, mode: initialModeOf(kind) };
    if (kind === 'office') {
