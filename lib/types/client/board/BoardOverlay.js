...
export function BoardOverlay({ useOverlay, useWorkspaces, remote, openSession, closeOverlay, t, }) {
    ...
    if (!open)
        return null;
    return (_jsxs(OverlayShell, { title: t('board.title'), closeLabel: t('board.close'), onClose: closeOverlay, testId: "board-overlay", actions: (_jsx("button", { type: "button", className: css.action, "data-testid": "board-create-toggle", onClick: () => { setCreating(value => !value); }, children: t('board.create') })), children: [error !== null && _jsx("p", { className: css.error, "data-testid": "board-error", children: t('board.error', { message: error }) }), creating && (_jsx(CreateForm, { workspaces: workspaces, t: t, onCancel: () => { setCreating(false); }, onCreate: (request) => {
                    setCreating(false);
