 ...
 */
export function applyReasoningToAll(draft, reasoningEfforts, listMode) {
    const next = { ...draft };
    if (listMode) {
        ...
        for (let index = 0; index < models.length; index++) {
            ...
            if (reasoningEfforts === undefined)
                delete updated['reasoningEfforts'];
            else
                updated['reasoningEfforts'] = reasoningEfforts;
 ...
 */
export function usableReasoningPreset(value) {
    if (value === false)
        return true;
    if (!isRecord(value))
        return false;
