 ...
 */
export function sessionCostOf(usage, pricing) {
    ...
    const cost = (usage.uncachedInputTokens * pricing.input
 ...
 */
export function formatUsdCost(cost) {
    if (cost === 0)
        return '$0.00';
 ...
 */
export function sessionCostCnyOf(usage, rate) {
    if (usage === undefined || rate === null || rate === undefined)
        return null;
    const billed = usage.uncachedInputTokens + usage.cacheReadTokens + usage.cacheWriteTokens + usage.outputTokens;
    if (billed <= 0)
        return null;
    const cost = (usage.uncachedInputTokens * rate.inputCacheMiss
 ...
 * @returns the prefixed display string.
 */
export function formatCnyCost(cost) {
    if (cost === 0)
        return '¥0.000';
