...
function DeepSeekLine({ view, pricing, rate, names, provider, model, usage, busy, now, fatal, t, onRefresh, }) {
    ...
    if (fatal !== null) {
        groups.push(_jsx("span", { className: css.error, children: t('balance.error', { message: fatal }) }, "fatal"));
    }
    else if (info === undefined && view.error !== undefined) {
        if (view.error.code === 'no-api-key') {
            groups.push(_jsx("span", { className: css.error, children: t('balance.noKey') }, "nokey"));
        }
        else {
            groups.push(_jsx("span", { className: css.error, children: t('balance.error', { message: view.error.message }) }, "berr"));
        }
    }
        ...
        groups.push(_jsxs("span", { className: css.group, title: title, children: [t('balance.title'), ' ', _jsx("b", { className: css.num, children: `¥${info.totalBalance.toFixed(2)}` }), low
                    ? _jsx("span", { className: css.warn, title: t('balance.low', { threshold: String(lowThreshold) }), children: " \u26A0" })
                    : null] }, "bal"));
        if (stale) {
...
function OpencodeGoLine({ names, model, view, loading, fatal, busy, now, t, onRefresh, }) {
    ...
    if (fatal !== null) {
        groups.push(_jsx("span", { className: css.error, children: t('balance.opencodeGoError', { message: fatal }) }, "fatal"));
    }
        ...
        groups.push(view.error.code === 'opencode-go-no-key'
            ? _jsx("span", { className: css.error, children: t('balance.opencodeGoNoKey') }, "nokey")
            : _jsx("span", { className: css.error, children: t('balance.opencodeGoError', { message: view.error.message }) }, "subberr"));
    }
    else {
