...
function PlainUserBubble({ content, loadImage, t }) {
    ...
    return (_jsxs("div", { className: css.userRow, "data-time-hover-root": true, children: [_jsxs("div", { className: css.userStack, children: [_jsx(ImageGallery, { images: images, load: loadImage, align: "end", labels: imageLabels(t) }), text !== '' && _jsx("div", { className: css.bubble, children: text }), rest.map((block, index) => _jsx("pre", { className: css.extraBlock, children: JSON.stringify(block, null, 2) }, index))] }), _jsx(CopyButton, { text: text, t: t })] }));
}
...
export const PastedTextUserNodeView = memo(function PastedTextUserNodeView({ node, loadImage, store, t, }) {
    ...
    return (_jsxs("div", { className: css.userRow, "data-time-hover-root": true, children: [_jsxs("div", { className: css.userStack, children: [_jsx(ImageGallery, { images: images, load: loadImage, align: "end", labels: imageLabels(t) }), text !== '' && (_jsxs("div", { className: css.bubble, children: [hit.start > 0 ? _jsx("span", { children: text.slice(0, hit.start) }) : null, _jsx(PastedTextChip, { hit: hit, store: store, t: t }), hit.end < text.length ? _jsx("span", { children: text.slice(hit.end) }) : null] })), rest.map((block, index) => _jsx("pre", { className: css.extraBlock, children: JSON.stringify(block, null, 2) }, index))] }), _jsx(CopyButton, { text: text, t: t })] }));
});
