...
import { MAX_OLDER_DOTS, NAV_HALF_WINDOW, NAV_WINDOW, navWindow, olderNodeCount, olderWindow } from "./window.js";
/**
...
`;
    ...
    const rowOfTurn = (turn) => {
        ...
        for (let i = 0; i < rows.length; i++) {
            if (turnOfUserIndex(all, rows, i) === turn)
                return rows[i] ?? null;
        }
        return null;
    };
    ...
    let disposeStats = () => { };
    const statsOf = () => {
        const raw = statsFace?.getSnapshot();
    ...
    const loadOneOlderPage = async () => {
        ...
        try {
            ...
            // A failed page stays represented by the same marker; the next click retries.
        }
    };
    ...
    const updateActiveClass = (active) => {
        ...
        dotsOf().forEach((dot) => {
            ...
            // the reading head.
            if (dot.hasAttribute('data-virtual-turn'))
                return;
            if (cursor + lo === active)
    ...
    const render = () => {
        ...
        const olderView = olderWindow(older, MAX_OLDER_DOTS);
        // Rebuild only when row identity or structure changed; scrolling moves
        // the active class alone.
        const expectedCount = (olderView.hidden > 0 ? 1 : 0) + olderView.visible
            + (lo > 0 ? 1 : 0) + (hi - lo + 1) + (hi < rows.length - 1 ? 1 : 0);
        ...
        // them, and a click pages backwards until the turn is materialized. A
        // session with more earlier turns than the dot budget folds the oldest
        // into one "load older" marker instead of one button per turn.
        if (olderView.hidden > 0) {
            ...
            more.addEventListener('click', () => { void loadOneOlderPage(); });
            bar.appendChild(more);
        }
    ...
    bar.addEventListener('wheel', (event) => {
        ...
        if (base < 0)
            return;
        // Up past the first MATERIALIZED row: keep the strip sliding by loading
        // the previous turn. The first rendered row's turn number gives an exact
        // target; when it is unknown, one older page request moves the loaded
        // window back and the follow-up scroll re-anchors the reading head.
        if (event.deltaY < 0 && base === 0) {
            ...
            if (firstTurn !== null && Number.isSafeInteger(firstTurn) && firstTurn > 1) {
                void jumpToTurn(firstTurn - 1);
                return;
            }
            const id = currentSessionId;
            const binding = id === null ? undefined : sessions.binding(id);
            const snapshot = binding?.session.getSnapshot();
            if (binding !== undefined && snapshot !== undefined && (snapshot.hasMore || snapshot.loadingOlder)) {
                void loadOneOlderPage();
            }
            return;
        }
