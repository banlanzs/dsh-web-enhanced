...
export function resolveWithin(root, rel) {
    if (rel.includes('\\'))
        throw new Error(`path '${rel}' must use forward slashes`);
    if (rel === '')
        return root;
    if (rel.startsWith('/') || /^[A-Za-z]:[\\/]/u.test(rel))
        throw new Error(`path '${rel}' must be relative`);
 ...
 */
export function compareFsEntries(left, right) {
    if (left.kind !== right.kind)
        return left.kind === 'dir' ? -1 : 1;
    return left.name < right.name ? -1 : 1;
}
/** One directory listing, skipping `.git` and the configured skip dirs. */
export async function listDirectory(root, rel, limits) {
    const dir = resolveWithin(root, rel);
    const entries = await readdir(dir, { withFileTypes: true });
    const out = [];
    for (const entry of entries) {
        if (entry.isDirectory() && (entry.name === '.git' || limits.skipDirs.includes(entry.name)))
            continue;
 ...
 */
export async function searchFiles(root, rel, query, limits) {
    ...
    const walk = async (dir, depth) => {
        if (depth > limits.searchMaxDepth || out.length >= limits.searchMaxEntries)
            return;
        const entries = (await readdir(dir, { withFileTypes: true })).sort(compareDirents);
        for (const entry of entries) {
            if (out.length >= limits.searchMaxEntries)
                return;
            if (entry.name === '.git')
                continue;
            const full = join(dir, entry.name);
            if (entry.isDirectory()) {
                if (limits.skipDirs.includes(entry.name))
                    continue;
 ...
 */
export async function readFileView(root, rel, limits) {
    const full = resolveWithin(root, rel);
    const info = await stat(full);
    if (info.isDirectory())
        return { error: { code: 'is-directory', message: `path '${rel}' is a directory` } };
 ...
 */
export async function countTextLines(root, rel, limits) {
    let handle;
    try {
        const full = resolveWithin(root, rel);
        const info = await stat(full);
        ...
        if (info.size === 0)
            return 0;
        if (info.size > limits.readMaxBytes)
            return null;
        ...
        if (buf.subarray(0, 8192).includes(0))
            return null;
        let lines = 0;
        for (let at = buf.indexOf(10); at !== -1; at = buf.indexOf(10, at + 1))
            lines += 1;
        if (buf.at(-1) !== 10)
            lines += 1;
        return lines;
...
export async function writeFileView(root, rel, content, limits) {
    const full = resolveWithin(root, rel);
...
export async function deleteFileView(root, rel) {
    const full = resolveWithin(root, rel);
    const info = await stat(full);
    if (info.isDirectory())
