...
| **Web 终端** | 「工作区」视图里的 **终端**标签页：直连宿主原生 PTY 注册表（`@deepseek-ai/dsh-terminal` + terminal-bash 后端）。会话由当前对话的 live agent 拥有（agent 销毁即清理），初始工作目录为工作区根目录；新建/切换/关闭会话，发送命令回车执行并返回渲染后的 viewport 与等待原因（stdin / 静默 / 超时 / 退出，与模型侧 terminal 工具同一契约），Ctrl+C 中断前台进程组，重开标签页自动重连该对话的存活会话并回读滚动历史。 |
