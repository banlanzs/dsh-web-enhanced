...
> DeepSeek Harness 的 Web 增强插件：任务看板（含 cron 定时执行）、Git 图谱、VSCode 式资源管理器（文件树 + 预览分栏）/变更面板、Web 终端、DeepSeek API 余额显示，以及纯文本模型的识图（图片理解）。
>
...
| **工作区视图** | 会话顶部视图栏中的「工作区」标签页，与「对话」「轨迹」并列，内含**资源管理器**（VSCode 式布局：左侧文件树侧边栏、右侧打开文件的预览）/ 变更 / **任务看板** / **Git 图谱** / **终端**五个面板。文件树侧边栏可一键收起/展开（状态持久化），支持整行展开、文件名搜索、点击在右侧打开预览；预览支持 markdown（含 GFM 表格、HTML 表格与行内 HTML）/ HTML（sandbox iframe）/ 代码 / **diff**（行级高亮 unified diff）/ CSV / 图片 / PDF / 文本 / **Office（docx/xlsx，宿主侧结构化转换）**，且支持**源码 / 分屏 / 预览**三态与保存；变更页基于真实 git status，支持 stage / unstage / discard 与逐文件 diff。当前面板与展开的目录按工作区持久化。
| **Web 终端** | 「工作区」视图里的 **终端**标签页：直连宿主原生 PTY 注册表（`@deepseek-ai/dsh-terminal` + terminal-bash 后端，两个包随插件依赖自动装进 profile 并由 patch 行挂载，无需手动组合）。会话由当前对话的 live agent 拥有（agent 销毁即清理），初始工作目录为工作区根目录；新建/切换/关闭会话，发送命令回车执行并返回渲染后的 viewport 与等待原因（stdin / 静默 / 超时 / 退出，与模型侧 terminal 工具同一契约），Ctrl+C 中断前台进程组，重开标签页自动重连该对话的存活会话并回读滚动历史。 |
