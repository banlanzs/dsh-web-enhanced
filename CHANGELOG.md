...
- Web 增强从设置的顶级标签移入「插件」页，注册为 `settings.plugins.tab`（id `web-enhanced`，order 20），与宿主的插件管理入口归拢在一起；宿主实现未改动。
- 「模型能力」不再与之并列，改为 Web 增强内部的一个标签。

### 修复：流式正文出现后折叠分组仍残留隐藏状态

- `assistant-step` 的 Markdown 可能通过原地更新文本节点流式输出；旧观察器只监听 `childList`，没有新 DOM 元素时不会重新判断该行是纯 Think 还是正式回复。现只对聊天 flow 内的 `characterData` 变化触发重算，页面其他文本变化不会额外刷新折叠器。
- render 改为每轮先计算完整的「期望隐藏集」再统一对账：流式正文让可隐藏行降到 2 以下、分组因而解散时，会同步移除折叠头和上一轮残留的 `data-we-tool-hidden`，正文与单条工具行都立即恢复可见。
- 新增回归用例：已折叠的 Think + Bash 分组中，原地流式补出正文后分组解散且无隐藏属性残留。
...
- 设置 → Web 增强已有 8 个标签，横向排列在窄窗口里放不下；现改为左侧 132px 竖向工具栏（左蓝色选中条 + 浅色选中背景），标签区自带纵向滚动，标签继续增多也不会挤压内容。右侧内容区改为 `flex: 1 + min-width: 0`，独立滚动。
- 修复：左右两栏此前都跟着设置外壳的唯一滚动条一起滚——根容器没有高度约束，内容较长的标签（识图 / 记忆）会把整页撑高，左栏也随之离开原位。现给 `.root` 加 `height: 100%; max-height: 100%`，外壳高度成为硬边界，左栏固定、只有右侧详情区出现自己的滚动条。测试 503 不变。

### 新增：工作完成的声音与系统弹窗通知

- 会话运行结束后自动提醒：客户端监听 `ctx.sessions.list` 快照流，跟踪每个会话 `running: true → false` 的完成边沿（与侧边栏绿色完成提示同源）；首次观测只记基线，打开页面不会回放旧完成，会话重新开始运行后再次结束会再次提醒。
- 提示音由 Web Audio 现场合成（两声短音，无需资源文件）：AudioContext 懒创建、每次播放前先 resume 再排程，符合浏览器自动播放策略；系统弹窗使用 Notification API，由操作系统显示在屏幕角落，点击弹窗回到对应的会话。
- 设置页新增「完成提醒」标签：总开关 / 完成提示音 / 系统弹窗 / 仅后台提醒，并提供「测试提示音」「测试系统弹窗」「请求通知权限」按钮（浏览器只允许在用户手势内请求通知权限）。偏好存 localStorage——纯浏览器本地行为，与皮肤选择一致，不需要新增宿主远程方法。
- 新增 `completion-notify.spec.ts` 22 个单测（设置恢复 / 完成边沿 / 开关门控 / 合成音排程 / Notification 适配器），测试 468 → 490。

## [0.20.1] - 2026-08-18

### 修复：关于页版本号与 npm 包内的构建机路径

- 修复：设置 → 关于页仍显示 0.19.0——`meta.ts` 的 `WEB_ENHANCED_VERSION` 与 `package.json` 靠约定手工同步，0.20.0 发布时漏了它。新增 `tests/meta.spec.ts` 把两者钉死，版本号不同步会直接挂掉 `pnpm check`（发布前强制门槛），不可能再发出去。
- 修复：npm 包内 `lib/client.js` 嵌有构建机的绝对路径（0.19.0 与 0.20.0 受影响，共 25 处 `//#region` 注释）——0.20.0 发布早于构建可复现性修复落地。本版产物由修复后的流水线构建：CSS 模块 id 与 lightningcss 的 `filename` 都用包内相对路径，不同机器构建的 `client.js` 字节一致，CI 的 lib/src 漂移门持续看护。
...
- 修复：shadow 渲染器接管全部 user 消息后丢失了宿主的**复制按钮**与**图片点击打开**——现在改为复用宿主同款原语：复制按钮使用 `Tooltip + IconCopyOutline16 + writeClipboard` 并带成功勾选反馈；图片使用宿主同一个 `ImageGallery`（缩略图点击打开原图、加载失败可重试），普通消息与粘贴文本 chip 行都带复制操作。
