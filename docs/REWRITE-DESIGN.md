...
**决策：任务看板与 Git 图谱这类覆盖型表面注册到 `shell.overlay`，不占用 `details`。**

**工作区表面（文件 / 预览 / 变更）的落位后来改了**：最初也放在 `shell.overlay`，做成右侧停靠浮层；现改为注册到 `conversation.view`，成为与「对话」「轨迹」并列的一个视图标签页。视图环一次只渲染一个条目、占满整列宽度，所以这个表面不再拥有任何几何——没有停靠、没有拖拽调宽、没有折叠，这些归框架管。代价是它激活时取代对话记录而非与之并排，收益是不遮挡内容、与宿主的视图切换一致，也不必和框架抢布局控制权。
